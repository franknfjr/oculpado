defmodule OculpadoWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: OculpadoWeb.Gettext
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(OculpadoWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(OculpadoWeb.Gettext, "errors", msg, opts)
    end
  end
  ...
  @doc """
  Rodapé com o link do projeto no GitHub (ícone SVG oficial).
  """
  attr :repo_url, :string, default: "https://github.com/franknfjr/oculpado"

  def github_footer(assigns) do
    ~H"""
    <footer class="mt-12 pb-8 text-center">
      <a
        ...
        class="inline-flex items-center gap-2 text-white/50 hover:text-white/90 transition-colors text-sm"
      >
  ...
  def share_icon(%{name: "whatsapp"} = assigns) do
    ~H"""
  ...
  def share_icon(%{name: "telegram"} = assigns) do
    ~H"""
    <svg class="share-ico" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  def share_icon(%{name: "facebook"} = assigns) do
    ~H"""
    <svg class="share-ico" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  def share_icon(%{name: "x"} = assigns) do
    ~H"""
    <svg class="share-ico" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  def share_icon(%{name: "link"} = assigns) do
    ~H"""
    <svg
      class="share-ico"
      viewBox="0 0 24 24"
